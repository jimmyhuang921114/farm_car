...
class MotorController(Node):
    def __init__(self):
        ...
        self.portHandler = PortHandler(DEVICE_NAME)#負責處理串口通信
        self.packetHandler = PacketHandler(PROTOCOL_VERSION)#負責打包和解包傳給馬達 
        self.init_dynamixel()#初始化
        self.speed_subscriber = self.create_subscription(SetVelocityDual, '/cmd_vel', self.speed_callback, 10)
    ...
    def speed_callback(self, msg):
        speed1 = -msg.left_motor_speed
        speed2 = msg.right_motor_speed
        self.get_logger().info(f'Speed1: {speed1}, Speed2: {speed2}')
        ...
        if dxl_comm_result1 != COMM_SUCCESS:
            rclpy.logging.get_logger('dynamixel_controller_dxl_comm_result1').info(self.packetHandler.getTxRxResult(dxl_comm_result1))
        elif dxl_error != 0:
            rclpy.logging.get_logger('dynamixel_controller').info(self.packetHandler.getRxPacketError(dxl_error))
        else:
            rclpy.logging.get_logger('dynamixel_controller').info("Set [ID: %d] [Goal Velocity: %d]" % (DXL_ID1, speed1))
        ...
        if dxl_comm_result2 != COMM_SUCCESS:
            ...
            rclpy.logging.get_logger('dynamixel_controller').info("Set [ID: %d] [Goal Velocity: %d]" % (DXL_ID2, speed2))
