...
class DistanceFilter(Node):
    def __init__(self):
        super().__init__('DistanceFilter')
        self.pub = self.create_publisher(Float32MultiArray, '/uwb/filter_data', 10)
    ...
    def callback(self, msg: Float32MultiArray):
        ...
        for idx, x in enumerate(raw):
            # 只把合法值推進 deque
            if not isnan(x):
                # 若 deque 裡已有值，就判斷是否跳太多
                if self.windows[idx]:
                    prev = self.windows[idx][-1]
                    if abs(x - prev) <= MAX_JUMP:
                        self.windows[idx].append(x)
                    else:
                        self.get_logger().warn(f"ID{IDS[idx]} 距離跳變過大，忽略此值: {x:.2f} (前值: {prev:.2f})")
                        # 不放進 deque
                else:
                    # deque 是空的，第一次直接放進去
                    self.windows[idx].append(x)
